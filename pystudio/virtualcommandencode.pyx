"""$Id: virtualcommandencode.pyx
$auth: Steve Torchinsky <steve.torchinsky@obspm.fr>
$created: Wed 28 Jun 13:44:15 CEST 2017
...
This is code extracted from dispatcheraccess.pyx originally by Pierre Chanial
These are methods which have moved by Wilfried Marty from TDispatcherAccess.h to TVirtucalCommandEncode.h
...
cdef class VirtualCommandEncode:
    """
    VirtualCommandEncode class.
    ...
    def sendSetAcqScienceMode(self, int asicNum):
        """
        sendSetAcqScienceMode(int asicNum)

        configuration de la carte NetQuic en mode science, par defaut la carte est dans ce mode (si asicNum = 0xFF, la commande est envoyée a tous les ASIC, si asic num < 16, la commande est envoyée a l'ASIC asicNum, pour envoyer à une liste d'ASICs utiliser les bits 8 à 23 pour specifier la liste, ex asicNum = 0x00FF00 configurera les asic 0 à 7)

        """
        cdef bool out = self._da.sendSetAcqScienceMode(asicNum)
        if not out:
            raise RuntimeError(self.lastError)

    def sendSetAcqTestPatternMode(self, int asicNum, int testMode):
        """
        sendSetAcqTestPatternMode(int asicNum, int testMode)

        configuration de la carte NetQuic en mode test pattern (si asicNum = 0xFF, la commande est envoyée a tous les ASIC, si asic num < 16, la commande est envoyée a l'ASIC asicNum, pour envoyer à une liste d'ASICs utiliser les bits 8 à 23 pour specifier la liste, ex asicNum = 0x00FF00 configurera les asic 0 à 7)

        """
        cdef bool out = self._da.sendSetAcqTestPatternMode(asicNum, testMode)
        if not out:
            raise RuntimeError(self.lastError)
    ...
    def sendSetRawModeList(self, int asicNum, rawList not None):
        """
        sendSetRawModeList(int asicNum, uint8[16] rawList)

        bascule en mode raw signal fixe (si asicNum = 0xFF, la commande est envoyée a tous les ASIC, si asic num < 16, la commande est envoyée a l'ASIC asicNum, pour envoyer à une liste d'ASICs utiliser les bits 8 à 23 pour specifier la liste, ex asicNum = 0x00FF00 configurera les asic 0 à 7)

        """
        rawList_ = np.ascontiguousarray(rawList, np.uint8)
        if rawList_.size != 16:
            raise ValueError("Expected array size of argument 'rawList' is '16'.")
        cdef quint8[::1] rawList__ = rawList_
        cdef bool out = self._da.sendSetRawModeList(asicNum, &rawList__[0])
        if not out:
            raise RuntimeError(self.lastError)
    ...
    def sendSetCalibTesPolar(self, int asicNum, int mode, int shape, int frequency, int amplitude, int offset):
        """
        sendSetCalibTesPolar(int asicNum, int mode, int shape, int frequency, int amplitude, int offset)
        ...
        """
        cdef bool out = self._da.sendSetCalibTesPolar(asicNum, mode, shape, frequency, amplitude, offset)
        if not out:
            raise RuntimeError(self.lastError)

    def sendSetCalibSquidPolar(self, int asicNum, int mode, int shape, int frequency, int amplitude, int offset):
        """
        sendSetCalibSquidPolar(int asicNum, int mode, int shape, int frequency, int amplitude, int offset)

        Specifie le signal de calib a injecter sur les Squids mode:0 pas de signal, 1 envoi du signal, shape: 0 sinus, 1 triangle, 2 continu  (si asicNum = 0xFF, la commande est envoyée a tous les ASIC, si asic num < 16, la commande est envoyée a l'ASIC asicNum, pour envoyer à une liste d'ASICs utiliser les bits 8 à 23 pour specifier la liste, ex asicNum = 0x00FF00 configurera les asic 0 à 7)

        """
        cdef bool out = self._da.sendSetCalibSquidPolar(asicNum, mode, shape, frequency, amplitude, offset)
        if not out:
            raise RuntimeError(self.lastError)
